...
const storageID = 'Mini-Conf-Papers-ICML';
let thumbnail_prefix = '/'

const persistor = new Persistor(storageID);
...
function updateKWs(keywords) {
    const kw = keywords.sort(d3.ascending)
    const allBtns = d3.select('#all_kw_show')
      .selectAll('.kw_list_item').data(kw)
      .join('button')
      .attr('class', 'btn btn-sm btn-outline-secondary kw_list_item')
      .text(d => d)
    allBtns.on('click', d => {
        window.open('papers.html?search_type=keyword&search='+encodeURIComponent(d))
    })
}
...
const card_image = (openreview, show) => {
    if (show) return ` <center><img class="lazy-load-img cards_img" data-src="${thumbnail_prefix}/${openreview.sourceid}-PaperThumbnail.jpg" width="80%"/></center>`
    else return ''
}
...
const card_html = openreview => `
        <div class="pp-card pp-mode-` + render_mode + ` ">
            ...
            <div class="checkbox-paper ${openreview.content.read ? 'selected' : ''}" style="display: block;position: absolute; bottom:35px;left: 35px;">✓</div>    
                <a href="/virtual/2020/poster/${openreview.id}"
                target="_blank"
                   class="text-muted">
